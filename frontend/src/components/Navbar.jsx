...
const Navbar = () => {
    ...
    return (
        <nav className="navbar navbar-expand-lg navbar-dark navbar-custom px-3 py-3">
            <Link className="navbar-brand fw-bold fs-4" to="/">Disaster Management</Link>
            <button className="navbar-toggler" type="button" data-bs-toggle="collapse" data-bs-target="#navbarNav">
                <span className="navbar-toggler-icon"></span>
            </button>
            <div className="collapse navbar-collapse" id="navbarNav">
                <ul className="navbar-nav ms-auto align-items-center">
                    <li className="nav-item"><Link className={`nav-link fs-5 ${isActive('/')}`} to="/">Home & Info</Link></li>
                    ...
                    {userInfo ? (
                        <>
                            ...
                            {userInfo.role === 'ADMIN' && (
                                <li className="nav-item"><Link className={`nav-link fs-5 text-warning ${isActive('/admin')}`} to="/admin">Admin Dashboard</Link></li>
                            )}
                            <li className="nav-item ms-lg-3 mt-2 mt-lg-0"><button className="btn btn-outline-light rounded-pill px-4" onClick={handleLogout}>Logout</button></li>
                        </>
                    ) : (
                        <>
